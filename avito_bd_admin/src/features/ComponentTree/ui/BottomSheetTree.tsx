...
const BottomSheetTree = () => {
  ...
  const handleDeleteBottomSheet = (id?: string) => {
    ...
    updateScreen((current) => {
      const next = {
        ...current,
        bottomSheets: (current.bottomSheets ?? []).filter(
          (bs) => bs._id !== selectedBottomSheetId
        ),
      };
      return next;
    });
  ...
  const bottomSheets: BottomSheetComponent[] =
    (screen as any).bottomSheets ?? [];

  const renderBottomSheet = (sheet: BottomSheetComponent, idx: number) => {
    const id = sheet._id;
    const hasChildren =
      Array.isArray(sheet.children) && sheet.children.length > 0;
    const opened = open.has(id);
          ...
          <div className={styles.children}>
            {sheet.children.map((child, cIdx) =>
              renderUiNode(
                ...
                `bottomSheets:${idx}/${child.type}:${cIdx}`
              )
            )}
          </div>
        )}
      ...
      <div className={styles.section}>
        ...
        <div className={styles.button_container}>
          ...
          <Button
            ...
            onClick={handleDeleteBottomSheet}
          >
            Удалить
          </Button>
        </div>
